param(
  ...
  [string]$Repo = $(if ($env:CODEX_REMOTE_REPO) { $env:CODEX_REMOTE_REPO } else { "zhuyidian/in-feishu" }),
...
Examples:
  ...
  & ([scriptblock]::Create((irm https://raw.githubusercontent.com/zhuyidian/in-feishu/main/install-release.ps1))) -Version v1.0.0
  & ([scriptblock]::Create((irm https://raw.githubusercontent.com/zhuyidian/in-feishu/main/install-release.ps1))) -InstallArgs '-instance','beta'
'@ | Write-Output
}
