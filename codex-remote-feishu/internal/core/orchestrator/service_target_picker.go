...
const targetPickerWorkspaceEntriesCacheTTL = defaultTargetPickerTTL
...
func (s *Service) handleTargetPickerPage(surface *state.SurfaceConsoleRecord, pickerID, fieldName string, cursor int, actorUserID string, answers map[string][]string) []eventcontract.Event {
	...
	case frontstagecontract.CardTargetPickerSessionFieldName:
		options := s.targetPickerSessionOptionsForRecord(surface, record, record.SelectedWorkspaceKey, record.Source, record.AllowNewThread)
...
func (s *Service) handleTargetPickerConfirm(surface *state.SurfaceConsoleRecord, pickerID, actorUserID, workspaceKey, sessionValue string, answers map[string][]string) []eventcontract.Event {
	...
	if kind, _ := parseTargetPickerSessionValue(requestedSessionValue); kind == control.FeishuTargetPickerSessionThread {
		clearTargetPickerRecordSelectionCaches(record)
	}
	view, err := s.buildTargetPickerView(surface, record)
	if err != nil {
		return notice(surface, "target_picker_unavailable", err.Error())
...
func (s *Service) buildTargetPickerView(surface *state.SurfaceConsoleRecord, record *activeTargetPickerRecord) (control.FeishuTargetPickerView, error) {
	...
	traceTargetPickerTiming := strings.HasPrefix(strings.TrimSpace(surface.SurfaceSessionID), "feishu:")
	traceTiming := func(stage string) {
		if !traceTargetPickerTiming {
			return
		}
		now := time.Now()
		log.Printf(
			"target picker timing: surface=%s picker=%s source=%s stage=%s selected_workspace=%s elapsed_ms=%d delta_ms=%d",
			strings.TrimSpace(surface.SurfaceSessionID),
			strings.TrimSpace(record.PickerID),
			record.Source,
			stage,
			strings.TrimSpace(record.SelectedWorkspaceKey),
			now.Sub(timingStart).Milliseconds(),
			now.Sub(timingLast).Milliseconds(),
		)
		timingLast = now
	}
	stage := record.Stage
	...
	if record.Source == control.TargetPickerRequestSourceWorktree {
		...
		traceTiming("filter_git_workspaces")
	}
	...
	if targetPickerUsesSessionSelection(record.Source) {
		sessionOptions = s.targetPickerSessionOptionsForRecord(surface, record, selectedWorkspace, record.Source, record.AllowNewThread)
		traceTiming("session_options")
	}
	...
	traceTiming("normalize_view")
	if traceTargetPickerTiming {
		log.Printf("target picker view state: surface=%s picker=%s source=%s page=%s selected_workspace=%s selected_session=%s session_options=%d workspace_options=%d can_confirm=%t confirm_label=%s allow_new_thread=%t",
			strings.TrimSpace(surface.SurfaceSessionID),
			strings.TrimSpace(record.PickerID),
			strings.TrimSpace(string(record.Source)),
			strings.TrimSpace(string(page)),
			strings.TrimSpace(selectedWorkspace),
			strings.TrimSpace(selectedSession),
			len(sessionOptions),
			len(workspaceOptions),
			canConfirm,
			strings.TrimSpace(confirmLabel),
			record.AllowNewThread,
		)
	}
...
func (s *Service) targetPickerWorkspaceEntries(surface *state.SurfaceConsoleRecord) []workspaceSelectionEntry {
	entries, _ := s.targetPickerWorkspaceEntriesAndVisibleViews(surface)
	return entries
}

func (s *Service) targetPickerWorkspaceEntriesAndVisibleViews(surface *state.SurfaceConsoleRecord) ([]workspaceSelectionEntry, []*mergedThreadView) {
	...
	return entries, visibleThreadViews
}
...
func (s *Service) targetPickerWorkspaceEntriesForRecord(surface *state.SurfaceConsoleRecord, record *activeTargetPickerRecord) []workspaceSelectionEntry {
	now := time.Now()
	if record != nil && len(record.WorkspaceEntriesCache) > 0 && now.Before(record.WorkspaceEntriesExpires) {
		return append([]workspaceSelectionEntry(nil), record.WorkspaceEntriesCache...)
	}
	entries, visibleThreadViews := s.targetPickerWorkspaceEntriesAndVisibleViews(surface)
	if record != nil {
		record.WorkspaceEntriesCache = append([]workspaceSelectionEntry(nil), entries...)
		record.WorkspaceEntriesExpires = now.Add(targetPickerWorkspaceEntriesCacheTTL)
		record.ThreadViewsCache = append([]*mergedThreadView(nil), visibleThreadViews...)
		record.ThreadViewsExpires = now.Add(targetPickerWorkspaceEntriesCacheTTL)
	}
	return entries
}

func clearTargetPickerRecordSelectionCaches(record *activeTargetPickerRecord) {
	if record == nil {
		return
	}
	record.WorkspaceEntriesCache = nil
	record.WorkspaceEntriesExpires = time.Time{}
	record.ThreadViewsCache = nil
	record.ThreadViewsExpires = time.Time{}
}
...
func (s *Service) targetPickerSessionOptions(surface *state.SurfaceConsoleRecord, workspaceKey string, source control.TargetPickerRequestSource, allowNewThread bool) []control.FeishuTargetPickerSessionOption {
	...
	return s.targetPickerSessionOptionsFromViews(surface, workspaceKey, source, allowNewThread, views)
}

func (s *Service) targetPickerSessionOptionsForRecord(surface *state.SurfaceConsoleRecord, record *activeTargetPickerRecord, workspaceKey string, source control.TargetPickerRequestSource, allowNewThread bool) []control.FeishuTargetPickerSessionOption {
	now := time.Now()
	views := []*mergedThreadView(nil)
	if record != nil && now.Before(record.ThreadViewsExpires) {
		views = append([]*mergedThreadView(nil), record.ThreadViewsCache...)
	}
	if views == nil {
		views = s.threadViewsVisibleInNormalList(surface, s.mergedThreadViews(surface))
		if record != nil {
			record.ThreadViewsCache = append([]*mergedThreadView(nil), views...)
			record.ThreadViewsExpires = now.Add(targetPickerWorkspaceEntriesCacheTTL)
		}
	}
	return s.targetPickerSessionOptionsFromViews(surface, workspaceKey, source, allowNewThread, views)
...
func (s *Service) targetPickerSessionOptionsFromViews(surface *state.SurfaceConsoleRecord, workspaceKey string, source control.TargetPickerRequestSource, allowNewThread bool, views []*mergedThreadView) []control.FeishuTargetPickerSessionOption {
	workspaceKey = normalizeWorkspaceClaimKey(workspaceKey)
	if workspaceKey == "" {
		return nil
	}
	...
	if targetPickerAllowsNewThread(source, allowNewThread) || (source == control.TargetPickerRequestSourceList && len(options) == 0 && s.targetPickerListCanOfferNewThreadForEmptyWorkspace(workspaceKey)) {
		options = append(options, control.FeishuTargetPickerSessionOption{
...
func (s *Service) targetPickerListCanOfferNewThreadForEmptyWorkspace(workspaceKey string) bool {
	return len(s.workspaceOnlineInstances(workspaceKey)) == 0
}
