...
func ParseMessageEvent(ctx context.Context, env InboundEnv, event *larkim.P2MessageReceiveV1) (control.Action, bool, error) {
	...
	case "text":
		...
		if !shouldHandleInboundGroupMessage(ctx, chatType, stringPtr(message.MessageType), stringPtr(message.Content), message.Mentions, env.BotOpenID, replyTargetMessageID, env.IsReplyTargetBot) {
