...
func (a *App) handleAction(ctx context.Context, action control.Action) *feishu.ActionResult {
	...
	traceCallbackTiming := false
	traceTiming := func(stage string) {
		if !traceCallbackTiming {
			return
		}
		now := time.Now()
		log.Printf(
			"surface action timing: surface=%s kind=%s message=%s stage=%s elapsed_ms=%d delta_ms=%d",
	...
	traceCallbackTiming = action.Kind == control.ActionTargetPickerSelectWorkspace ||
		action.Kind == control.ActionTargetPickerSelectSession ||
		(action.Inbound != nil && strings.TrimSpace(action.Inbound.CardDaemonLifecycleID) != "")
	...
	traceTiming("apply_ingress")
	contract := control.ResolveFeishuFrontstageActionContract(action)
	inlineResult, appendEvents := a.synchronousCurrentCardActionResultLocked(action, contract, events)
	traceTiming("inline_result")
	inlineNavigationReplace := inlineResult != nil && contract.CurrentCardMode == control.FeishuFrontstageCurrentCardInlineView
	if !inlineNavigationReplace || len(appendEvents) != 0 {
		a.handleUIEventsLocked(ctx, appendEvents)
	}
	traceTiming("handle_ui_events")
	a.maybeReplayPendingRequestVisibilityAfterActionLocked(ctx, action)
	traceTiming("replay_visibility")
	...
	traceTiming("sync_surface_resume")
	a.syncClaudeWorkspaceProfileStateLocked()
	traceTiming("sync_workspace_profile")
	a.syncWorkspaceSurfaceContextFilesLocked()
	traceTiming("sync_context_files")
