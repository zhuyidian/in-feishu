...
- `v1.7.3`：群聊里支持“引用飞书文件 + @机器人指令”的处理链路；机器人会下载被引用文件并把本地路径注入给 Codex，同时要求替换同名项目文件前先列出候选路径并等待确认。
- 本次修复了 `gkprep-build-apk` 直接执行通道在 Windows 下的路径和编码问题：去掉 `//?/` 扩展路径对 PowerShell skill 脚本的影响，统一 UTF-8 输出，避免飞书卡片出现乱码或 `$root` 为空导致 APK 打包失败。
- `v1.7.2`：选中工作区后会读取该工作区 `.agents/skills`，命中 `gkprep-build-apk` 这类 APK 打包请求时直接走项目 skill 执行通道，不再先交给普通 Codex 会话兜底。
- 本次同时收敛了 APK 打包结果回写：APK 文件仍由项目 skill 发送到飞书群，codex-remote 只保留简短执行状态，避免同一次构建日志和文件预览重复出现。
